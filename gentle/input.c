...
/*  Input Files                                                               */
/*----------------------------------------------------------------------------*/
void close_string(char **ref_str);
void app_to_string(char ch);
...
static FILE *SourceFile() { return InFile; }

/*----------------------------------------------------------------------------*/

char *SourceName() { return InFileName; }

/*----------------------------------------------------------------------------*/

DefSourceName(str) char *str;
{
...
GetSourceName(str) char **str;
{ *str = InFileName; }

/*----------------------------------------------------------------------------*/

static FILE *open_file(unit) char *unit;
{
  char buf[1000];
  FILE *InFile;
...
static long is_defined(path) char *path;
{
  long i;
...
static long more_files() { return (filecount > CurFile); }
...
#define RECOG(x)                                                               \
  {                                                                            \
    bufptr = BUFPTR;                                                           \
    yylval.attr[0] = CurFile * MaxLines * MaxCols + CurLine * MaxCols +        \
                     (BUFPTR - firstcolptr);                                   \
    return (x);                                                                \
  }
    ...
    for (;;) {
      if (CH == '\'') {
        ...
        if (strcmp(string, "MODULE") == 0)
          ...
          RECOG(TYPETOKEN)
        else if (strcmp(string, "ACTION") == 0)
          RECOG(PROC)
        else if (strcmp(string, "action") == 0)
          RECOG(PROC)
        else if (strcmp(string, "CONDITION") == 0)
          RECOG(COND)
        else if (strcmp(string, "condition") == 0)
          RECOG(COND)
        else if (strcmp(string, "PREDICATE") == 0)
          RECOG(COND)
        else if (strcmp(string, "predicate") == 0)
          RECOG(COND)
        else if (strcmp(string, "SWEEP") == 0)
          RECOG(SWEEP)
        else if (strcmp(string, "sweep") == 0)
          RECOG(SWEEP)
        else if (strcmp(string, "NONTERM") == 0)
          RECOG(NONTERMTOKEN)
        else if (strcmp(string, "nonterm") == 0)
          RECOG(NONTERMTOKEN)
          ...
          RECOG(TOKENTOKEN)
        else if (strcmp(string, "CHOICE") == 0)
          RECOG(CHOICE)
        else if (strcmp(string, "choice") == 0)
          RECOG(CHOICE)
        else if (strcmp(string, "RULE") == 0)
          RECOG(RULETOKEN)
        else if (strcmp(string, "rule") == 0)
          RECOG(RULETOKEN)
