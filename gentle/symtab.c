...
static allocate_idstringtab() {
  idstringtab_ptr = (char *)malloc(STRINGTAB_PIECE + STRINGTAB_EXTRA);
...
static allocate_idtab() {
  idtab_ptr =
      (struct IdentRec *)malloc(sizeof(IDTAB /*struct IdentRec [IDTABSIZE]*/));
  if (idtab_ptr == 0) {
...
void id_to_string(Ident id, char **repr)
{ *repr = id->firstposptr; }
...
DefLocalMeaning(id, m1, m2) Ident id;
long m1;
long m2;
{
  id->LocalMeaning = m1;
...
long GetGlobalMeaning(id, m) Ident id;
long *m;
{
...
long GetExportFlag(id, f) Ident id;
long *f;
{
  if (id->GlobalMeaning == 0)
    return 0;
...
long GetFunctorMeaning(id, m) Ident id;
long *m;
{
...
long GetLocalMeaning(id, m) Ident id;
long *m;
{
...
long GetLocalMeaning2(id, m) Ident id;
long *m;
{
  if (id->LocalMeaning == 0)
    return 0;
...
long HasGlobalMeaning(id) Ident id;
{ return id->GlobalMeaning != 0; }

/*----------------------------------------------------------------------------*/

long HasLocalMeaning(id) Ident id;
...
enter_keyword(class, idstart) int class;
char *idstart;
{
  Ident NewId;
...
UniqueTokenId(ref_id) Ident *ref_id;
{
  char buffer[100];
  tokencount++;
...
append_to_buf(str) char *str;
{
  char *p;
  p = str;
  while (*p != 0) {
    *namebufptr = *p;
    if (namebufptr == stop_namebuf)
      resize_namebuf();
    else
      namebufptr++;
    p++;
...
CreateNameForToken(Str, ref_id) char *Str;
Ident *ref_id;
{
  ...
  if (is_word(Str)) {
    ...
    while (*p != 0) {
      ...
      case ' ':
        append_to_buf("_BLANK");
        break;
      case '!':
        append_to_buf("_EXCLAM");
        break;
      case '"':
        append_to_buf("_DQUOTE");
        break;
      case '#':
        append_to_buf("_SHARP");
        break;
      case '$':
        append_to_buf("_DOLLAR");
        break;
      case '%':
        append_to_buf("_PERCENT");
        break;
      case '&':
        append_to_buf("_AMPERSAND");
        break;
      case '\'':
        append_to_buf("_BACKSLAH");
        break;
      case '(':
        append_to_buf("_LPAREN");
        break;
      case ')':
        append_to_buf("_RPAREN");
        break;
      case '*':
        append_to_buf("_ASTERISK");
        break;
      case '+':
        append_to_buf("_PLUS");
        break;
      case ',':
        append_to_buf("_COMMA");
        break;
      case '-':
        append_to_buf("_MINUS");
        break;
      case '.':
        append_to_buf("_DOT");
        break;
      case '/':
        append_to_buf("_SLASH");
        break;
      case ':':
        append_to_buf("_COLON");
        break;
      case ';':
        append_to_buf("_SEMICOLON");
        break;
      case '<':
        append_to_buf("_LESS");
        break;
      case '=':
        append_to_buf("_EQUAL");
        break;
      case '>':
        append_to_buf("_GREATER");
        break;
      case '?':
        append_to_buf("_QUESTIONM");
        break;
      case '@':
        append_to_buf("_ATSIGN");
        break;
      case '[':
        append_to_buf("_LBRACKET");
        break;
      case '\\':
        append_to_buf("_BACKSLASH");
        break;
      case ']':
        append_to_buf("_RBRACKET");
        break;
      case '^':
        append_to_buf("_POWER");
        break;
      case '_':
        append_to_buf("_UNDERSCORE");
        break;
      case '`':
        append_to_buf("_BACKQUOTE");
        break;
      case '{':
        append_to_buf("_LBRACE");
        break;
      case '|':
        append_to_buf("_BAR");
        break;
      case '}':
        append_to_buf("_RBRACE");
        break;
      case '~':
        append_to_buf("_TILDE");
        break;
      default: {
        char rep[5];
        sprintf(rep, "_%d", *p);
